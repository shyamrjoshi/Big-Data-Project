...
subwayData = sc.textFile(sys.argv[1], 1, use_unicode=False)
...
#
# taxirdd = taxiData.map(lambda x: x.split(',')).map(convertdate)
# weatherrdd = weatherData.map(lambda x: x.split(',')).map(lambda x: x[2]+','+x[12]+','+x[16]+','+x[17]).filter(filterweather)
# taxiweathercross = taxirdd.cartesian(weatherrdd)
# taxiweathercrossfilter = taxiweathercross.filter(filtercross)
# taxi_weather_rain_subway_join = subwayData.cartesian(taxiweathercrossfilter)
# taxi_weather_rain_subway_distance = taxi_weather_rain_subway_join.map(calculateDistanceFromSubways).reduceByKey(getMinDist)
# subwaycount = subwayData.map(countMap).reduceByKey(lambda x,y: x+y).takeOrdered(10, key = lambda x: -x[1])
subwaycount = subwayData.filter(dataclean).map(countMap).reduceByKey(lambda x,y: x+y).takeOrdered(10, key = lambda x: -x[1])


# taxiweather_rain_subway = taxiweathercrossfilter.map(getNearestSubway)
# taxiweather_rain_subwaydfilter = taxiweather_rain_subway.filter(subwayDistanceFilter)
# taxiweather_rain_subwaydfiltercount = taxiweather_rain_subwaydfilter.map(lambda x: (x[0],1)).reduceByKey(lambda x,y: x+y)

# output = subwaycount.collect()
output = subwaycount
