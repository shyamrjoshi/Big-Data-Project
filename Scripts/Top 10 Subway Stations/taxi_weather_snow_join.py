...
def filterweather(line):
    ...
    if (mw in range(20, 40) or aw1 in range(20, 40) or aw2 in range(20, 40)):
        return True
    else:
        return False
...
def convertdate(line):
    line = line.split(',')
    pickuptaxidatetime = line[0]

    key = getTimeInterval(pickuptaxidatetime)
    value = line[7]+','+line[8]
    return (key,value)
...
# subwaycount = taxi_weather_rain_subway_distance.map(countMap).reduceByKey(lambda x,y: x+y)
...
output = taxiweatherjoin.collect()

for line in output:
    splits = line[1][0].split(',')
    long = splits[0]
    lat = splits[1]
    print('%s,%s'%(long,lat))
